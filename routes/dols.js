...
router.put('/:id', writeRateLimiter, requireAuth, requireHomeAccess, requireModule('compliance', 'write'), async (req, res, next) => {
  try {
    ...
    // Redact description + mental-capacity narrative — DoLS records are Article 9
    // by definition (mental capacity). The fact of change is auditable, not the text.
    const changes = diffFields(existing, record, { extraSensitive: ['description', 'best_interest_decision', 'decision_area_detail'] });
