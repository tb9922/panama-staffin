...
export function getIntakeStoredPath(homeId, intakeItem) {
  return buildIntakePath(homeId, intakeItem.stored_name);
}
...
async function runOcrAndPersist(homeId, { file, createdBy, existingIntakeId = null }) {
  const fileBuffer = await readFile(file.path);
  const fileSha = sha256(fileBuffer);
  const duplicate = await documentIntakeRepo.findBySha(homeId, fileSha);
  if (duplicate) {
    if (existingIntakeId && duplicate.id === existingIntakeId) {
      // allow re-processing the same item on retry
    } else {
  ...
  try {
    ...
    if (existingIntakeId) {
      return documentIntakeRepo.update(existingIntakeId, homeId, {
    ...
    const failed = await documentIntakeRepo.create(homeId, {
      status: 'failed',
      ...basePayload,
      created_by: createdBy,
      error_message: err.message,
    ...
    failed._ocrError = { message: err.message, statusCode: err.statusCode || 502 };
    return failed;
  }
}

export async function createScanIntake(homeId, { file, createdBy }) {
  return runOcrAndPersist(homeId, { file, createdBy });
}

export async function retryScanIntake(homeId, intakeId) {
  const intakeItem = await documentIntakeRepo.findById(intakeId, homeId);
  if (!intakeItem) throw Object.assign(new Error('Scan item not found'), { statusCode: 404 });
  const file = {
    path: buildIntakePath(homeId, intakeItem.stored_name),
    filename: intakeItem.stored_name,
    originalname: intakeItem.original_name,
    mimetype: intakeItem.mime_type,
    size: intakeItem.size_bytes,
  };
  return runOcrAndPersist(homeId, {
    file,
    createdBy: intakeItem.created_by,
    existingIntakeId: intakeItem.id,
  });
}
