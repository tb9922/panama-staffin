...
const LINE_COLS = `id, payroll_run_id, staff_id,
  ...
  authorised_absence_hours, authorised_absence_pay,
...
function shapeLine(row) {
  ...
  return {
    ...
    authorised_absence_hours: f(row.authorised_absence_hours),
    authorised_absence_pay: f(row.authorised_absence_pay),
...
export async function updateLineDeductions(lineId, homeId, data, client) {
  ...
  const { rows } = await conn.query(
    `UPDATE payroll_lines SET
       ...
       authorised_absence_hours = $4,
       authorised_absence_pay   = $5,
       ssp_days             = $6,
       ssp_amount           = $7,
       enhanced_sick_amount = $8,
       pension_employee     = $9,
       pension_employer     = $10,
       tax_deducted         = $11,
       employee_ni          = $12,
       ...
       net_pay              = $16
     WHERE id = $17
       AND payroll_run_id IN (SELECT id FROM payroll_runs WHERE home_id = $18)
     RETURNING ${LINE_COLS}`,
    [
      data.holiday_days ?? 0,
      data.holiday_pay ?? 0,
      data.holiday_daily_rate ?? null,
      data.authorised_absence_hours ?? 0,
      data.authorised_absence_pay ?? 0,
      data.ssp_days ?? 0,
      data.ssp_amount ?? 0,
