...
describe('localDates', () => {
  describe('todayLocalISO', () => {
    ...
    it('accepts a date string', () => {
      // Strings are parsed via new Date(string) — local tz interpretation
  ...
  describe('startOfLocalWeekISO', () => {
    it('returns Monday for a Wednesday', () => {
      ...
      expect(startOfLocalWeekISO(new Date(2026, 3, 8))).toBe('2026-04-06');
    });

    it('returns Monday for a Monday', () => {
      expect(startOfLocalWeekISO(new Date(2026, 3, 6))).toBe('2026-04-06');
    });

    it('returns previous Monday for a Sunday', () => {
      // 2026-04-12 is a Sunday
      expect(startOfLocalWeekISO(new Date(2026, 3, 12))).toBe('2026-04-06');
    });

    it('crosses month boundary backwards', () => {
      // 2026-05-01 is a Friday
      expect(startOfLocalWeekISO(new Date(2026, 4, 1))).toBe('2026-04-27');
    });
  });
  ...
  describe('currentLocalYearMonth', () => {
    it('returns YYYY-MM format', () => {
      expect(currentLocalYearMonth(new Date(2026, 3, 10))).toBe('2026-04');
    });

    it('pads single-digit month', () => {
      expect(currentLocalYearMonth(new Date(2026, 0, 1))).toBe('2026-01');
    });

    it('handles December', () => {
      expect(currentLocalYearMonth(new Date(2026, 11, 25))).toBe('2026-12');
    });
  });
});
