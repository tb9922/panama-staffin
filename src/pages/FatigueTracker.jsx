  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className={`rounded-lg p-4 ${exceeded.length > 0 ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
        ...
        <div className={`rounded-lg p-4 ${atRisk.length > 0 ? 'bg-amber-50 border border-amber-200' : 'bg-green-50 border border-green-200'}`}>
      ...
      <div className="bg-white rounded-lg shadow overflow-x-auto">
        <table className="text-[11px] border-collapse">
          ...
          <tbody>
            {fatigueData.map(s => (
              <tr key={s.id} className={`border-b ${s.fatigue.exceeded ? 'bg-red-50' : s.fatigue.atRisk ? 'bg-amber-50' : ''}`}>
                ...
                <td className="py-1 px-2 text-center font-mono">{s.totalWorking}/28</td>
                <td className="py-1 px-2 text-center font-mono">{s.avgWeeklyShifts.toFixed(1)}</td>
              </tr>
            ))}
          </tbody>
        </table>
