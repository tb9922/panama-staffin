...
export default function AuditLog() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      showToast({ title: 'Audit log exported', message: 'Excel workbook downloaded' });
    } catch (e) {
      setError(e.message);
    } finally {
      setExporting(false);
  ...
  if (loading) {
    return <div className={PAGE.container}><LoadingState message="Loading audit log..." /></div>;
  }
  ...
  if (error && log.length === 0) {
    return (
      <div className={PAGE.container}>
        <ErrorState
          title="Audit log needs attention"
          message={error}
          onRetry={load}
        />
      </div>
    ...
    <div className={PAGE.container}>
      <div className={PAGE.header}>
        <div>
          ...
          <p className="text-sm text-gray-500">Last 100 actions — who changed what and when.</p>
        </div>
      ...
      {error && <ErrorState title="Audit export needs attention" message={error} onRetry={load} className="mb-4" />}

      <div className={CARD.flush}>
        {log.length === 0 ? (
          <EmptyState
            title="No audit entries yet"
            description="Audit history will appear here as users sign in and make changes."
            compact
            className="px-4"
          />
        ) : (
