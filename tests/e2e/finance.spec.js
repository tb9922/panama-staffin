...
test.describe('Finance Module', () => {
  test('Finance Dashboard renders KPIs', async ({ page }) => {
    ...
    await expect(page.getByText(/Finance/i)).toBeVisible({ timeout: 15_000 });
    // Should show financial KPI cards or summary
  ...
  test('Income page loads with table', async ({ page }) => {
    ...
    await expect(page.getByText(/Income|Billing|Invoice/i)).toBeVisible({ timeout: 15_000 });
  ...
  test('Expenses page loads with table', async ({ page }) => {
    ...
    await expect(page.getByText(/Expense/i)).toBeVisible({ timeout: 15_000 });
    const table = page.locator('table');
    await expect(table).toBeVisible({ timeout: 10_000 });
  });
