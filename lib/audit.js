...
//
// NOTE: `description` was removed from this list because it blinded the audit
// trail on the most-edited field across incidents/complaints/policies/risks/
// whistleblowing/maintenance/CQC evidence. The audit log is itself access-
// controlled (home_manager / deputy_manager only) so showing description text
// in audit changes is acceptable. PII inside description (resident names,
// staff names) is separately redacted via `person_affected_name`,
// `raised_by_name`, etc. If a future module needs `description` redacted, pass
// an additional set into `diffFields(before, after, { extraSensitive })`.
const SENSITIVE_FIELDS = new Set([
  ...
  // Article 9 — keep `condition_description` redacted (HR EDI medical conditions)
 ...
 *   to redact in the diff output. Use this from a module that legitimately
 *   wants to keep its `description` (or other normally-visible field) out of
 *   the audit log. Defaults to none.
 * @returns {Array<{ field: string, old: *, new: * }>}
 */
export function diffFields(before, after, options = {}) {
  if (!after) return [];
  const extraSensitive = options.extraSensitive
    ? new Set(options.extraSensitive)
    : null;
  const isSensitive = (key) =>
    SENSITIVE_FIELDS.has(key) || (extraSensitive && extraSensitive.has(key));
  const changes = [];
