...
const isTestBypassEnabled = process.env.NODE_ENV === 'test'
  && (process.env.VITEST === 'true' || process.env.DISABLE_RATE_LIMITS === '1');

/** Key generator: IP + username when authenticated, IP-only otherwise */
export function perUserKey(req) {
  const ip = ipKeyGenerator(req.ip);
  if (req.user?.username) return `${ip}:${req.user.username}`;
  ...
  if (token) {
    try {
      ...
      if (decoded?.username) return `${ip}:${decoded.username}`;
    } catch {
      // Invalid tokens fall back to the coarse IP bucket; requireAuth rejects them later.
    }
  }
  return ip;
}
...
export const writeRateLimiter = isTestBypassEnabled ? passthrough : rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 500,
...
export const readRateLimiter = isTestBypassEnabled ? passthrough : rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 2000,
