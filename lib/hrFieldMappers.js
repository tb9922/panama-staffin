...
export function mapFlexFields(data) {
  ...
  if (m.status === 'withdrawn' && (!('decision' in m) || m.decision == null || m.decision === '')) m.decision = 'withdrawn';
  if (m.decision === 'withdrawn' && (!('status' in m) || m.status == null || m.status === '')) m.status = 'withdrawn';
  if (m.appeal_outcome === '') m.appeal_outcome = null;
  if (m.notes === '') m.notes = null;
