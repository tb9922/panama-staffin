...
export function mapEdiFields(data) {
  const m = { ...data };
  if ('date_recorded' in m && !('complaint_date' in m)) { m.complaint_date = m.date_recorded; delete m.date_recorded; }
  if ('category' in m && !('harassment_category' in m) && !('description' in m)) {
    if (m.record_type === 'reasonable_adjustment') m.description = m.category;
    else m.harassment_category = m.category;
    delete m.category;
  }
...
export function mapRenewalFields(data) {
  const m = { ...data };
  ...
  if ('certificate_number' in m && isDbs && !('dbs_certificate_number' in m)) {
