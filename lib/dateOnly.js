...
export function endOfLocalMonthISO(date = new Date(), monthOffset = 0) {
  const { year, month } = toDateOnlyParts(date);
  return todayLocalISO(new Date(year, month + monthOffset + 1, 0));
}

function toUtcMidnightMillis(date) {
  const { year, month, day } = toDateOnlyParts(date);
  return Date.UTC(year, month, day);
}

export function diffLocalISODays(targetDate, referenceDate = todayLocalISO()) {
  return Math.round((toUtcMidnightMillis(targetDate) - toUtcMidnightMillis(referenceDate)) / 86400000);
}
