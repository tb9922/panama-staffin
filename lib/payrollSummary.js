...
export function generateSummaryPDF(run, lines, home) {
  ...
  const authorisedAbsencePay = sum(lines, 'authorised_absence_pay');
  ...
  const grossRows = [
    ...
    authorisedAbsencePay > 0 && ['Paid Authorised Absence', fmtGbp(authorisedAbsencePay)],
  ...
  doc.text(
    `Generated by Panama Staffing on ${todayLocalISO()}. Official figures from your payroll software.`,
    margin, y,
  );
